...
<head>
    ...
    <link rel="stylesheet"
        href="https://demo-instashop.myinsales.ru/served_assets/checkout2-ae84901bb11bcd966528145ff2cd7305a7aaed034ca6954f4798438f5045f1a9.css">
    <link rel="stylesheet" href="https://assets3.insales.ru/assets/1/1041/1336337/1582725497/custom_checkout2.css">
                ...
                <div class="cell-3 text-right cell-6-m">
                    <div class="header-menu-right row flex-middle flex-end">
                        <div class="hide-sm m-l-20">
                            <a class="login-link" href="{{ auth()->check() ? '/admin' : route('login') }}">
                        ...
                        <div class="m-l-20">
                            <a class="js-basket hover-opacity" href="{{ route('instashop.checkout') }}"
                ...
                <div class="cell-6 hide-m">
                    <ul class="no-list-style p-0 m-t-0 m-b-0">
                        ...
                        @foreach ($categories as $category)
                            <li class="inline-middle m-l-10 m-r-10">
                                <a class="bttn-underline "
                                    href="{{ route('instashop.category', $category) }}">{{ $category->name }}</a>
                            </li>
                        @endforeach
                    </ul>
                </div>
...
<div class="co-section--checkout_content">
    <div class="co-section--checkout_order co-section--checkout_order_sided">
        ...
        <div class="co-sidebar-wrapper"><button class="co-sidebar-toggler js-co-sidebar-toggler"><span><span
                        class="co-icon halfling-shopping-cart"></span><span class="co-sidebar-toggler_title">Ваш
                        заказ</span><span class="co-icon halfling-menu-down"></span></span><span
                    class="co-basket_total-price co-price--current">2500&nbsp;руб</span></button>
                ...
                <div class="co-basket co-checkout-block--padded">
                    <div class="co-basket_item-list">
                        ...
                        <div class="co-basket_item">
                            <div class="co-basket_item-image_container">
                                <div class="co-basket_item-image"><img alt="image"
                                        src="{{ $item->image }}">
                                </div>
                            </div>
                            <div class="co-basket_item-description">{{ $item->name }}</div>
                            <div class="co-basket_item-total"><span class="co-basket_item-count">{{ $cart->products()->where('product_id', $item->id)->count() }}</span> x <span
                                    class="co-basket_item-price co-price--current">{{ $item->price_format }}</span></div>
                        </div>
                        @endforeach
                    ...
                    <div class="co-basket_subtotal-list">
                        <div class="co-basket_subtotal">
                            ...
                            <div class="co-basket_subtotal-price co-price--current" id="items_price">{{ $cart->subtotal }}</div>
                            <div id="order_items_price" style="display:none">{{ $cart->subtotal }}</div>
                        </div>
                        ...
                        <div class="co-basket_subtotal">
                            ...
                            <div class="co-basket_subtotal-price co-price--current" id="delivery_price">$0</div>
                        ...
                        <div class="co-basket_subtotal" id="payment_way_block" style="display:none;">
                            ...
                            <div class="co-basket_subtotal-price co-price--current" id="payment_gateway_price">
                                0&nbsp;руб</div>
                        </div>
                    </div>
                    <div class="co-basket_total">
                        <div class="co-basket_total-title">Итого:</div>
                        <div class="co-basket_total-price co-price--current" id="total_price">{{ $cart->subtotal }}</div>
                    </div>
                </div>
            </div>
        </div>
        <div class="co-notice--danger co-notice--flash" id="checkout_order_errors" style="display: none"></div>
        <form id="order_form" class="co-checkout-order_form co-checkout-block--padded"
            action="{{ route('instashop.buy', $cart) }}"  method="post">
            @csrf
            ...
            <div class="co-delivery_method-list co-checkout-block">
                <div class="co-checkout-block">
                    <div class="co-input co-input--required co-input--tel">
                        <h2 class="co-input-label co-title co-title--h2">Контактный телефон</h2>
                        <input
                            class="co-input-field js-input-field" type="tel" id="client_phone" name="phone"
                            value="">
                    </div>
                <div class="co-customer co-checkout-block co-tabs" data-tabs-organization="false"
                    data-tabs-person="true" data-tabs-registration="true" id="customer-fields">
                        <div
                            class="co-input co-input--required co-input--text co-input--name  co-input--nested co-input--empty_nested">
                            <label class="co-input-label" for="client_name">Контактное лицо (ФИО)</label><input
                                class="co-input-field js-input-field" autocomplete="off" type="text" id="client_name"
                                name="name" value="">
                        ...
                        <div class="co-input  co-input--text co-input--email  co-input--nested co-input--empty_nested">
                            ...
                            <input
                                class="co-input-field js-input-field" autocomplete="off" type="text" id="client_email"
                                name="email" value="">
                            <div class="co-input-notice co-notice--danger">Поле не заполнено</div>
                        </div>
                    </div>
            ...
            <div class="consent_to_personal_data_checkbox co-checkout-block"></div><button
                class="co-button co-button--checkout js-button-checkout_submit" id="create_order"
                type="submit">Подтвердить заказ</button>
        </form>
        <div id="picker-container"></div>
        <div class="co-modal co-modal--login co-modal--fixed co-modal--hide">
            <div class="co-modal-wrapper"><button
                    class="co-modal-close co-icon halfling-remove js-modal-close"></button>
                <h3 class="co-modal-title co-title co-title--h2">Для постоянных покупателей</h3>
                <div class="co-form--login co-modal-login_form js-modal-body">
                    <div class="co-input co-input--required co-input--email"><input
                            class="co-input-field js-input-field" name="email" placeholder="Email" type="text"></div>
                    <div class="co-input co-input--required co-input--password"><input
                            class="co-input-field js-input-field" name="psw" placeholder="Пароль" type="password">
                        <div class="co-input-notice co-notice--danger">Сочетание логина и пароля не подходит</div>
                    </div><button class="co-modal-button co-button js-modal-submit--login"
                        data-checkout-path="/new_order" data-login="/client_account/session"
                        type="submit">Войти</button><a class="co-button--password_recover"
                        href="/client_account/password/change">Восстановить пароль</a>
                </div>
            </div>
        </div>
        <script>
            window.CURRENCY_FORMAT =
                "{\"delimiter\":\"\",\"separator\":\".\",\"format\":\"%n %u\",\"unit\":\"руб\",\"show_price_without_cents\":0}"
        </script>
    </div>
</div>
